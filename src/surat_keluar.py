...
class SuratKeluar(QWidget):
    def __init__(self):
        ...
        self.filtered_data = [] # Data setelah difilter cari
        self.current_page = 1
    ...
    def init_ui(self):
        ...
        self.search_input.setPlaceholderText("🔍 Cari Nomor Surat atau Perihal...")
        ...
        self.table.setHorizontalHeaderLabels(["NO", "NO. SURAT", "PERIHAL", "TUJUAN", "TANGGAL", "AKSI"])
        ...
        style_nav = "QPushButton { background:#dfe4ea; border-radius:5px; padding:5px 15px; font-weight:bold; } QPushButton:disabled { color:#a4b0be; }"
    ...
    def display_data(self, data):
        ...
        for i, row in enumerate(page_data):
            ...
            no_item.setData(Qt.ItemDataRole.UserRole, row[0]) # Simpan ID
    ...
    def load_data(self):
        try:
            ...
            if db:
                cursor = db.cursor()
                cursor.execute("SELECT id, nomor_surat, judul_surat, asal_surat, tanggal, file_path FROM surat WHERE kategori='keluar' ORDER BY id DESC")
    ...
    def filter_data(self):
        ...
        filtered = [d for d in self.all_data if text in str(d[1]).lower() or text in str(d[2]).lower()]
        self.current_page = 1
    ...
    def export_to_excel(self):
        ...
        if path:
            try:
                df = pd.DataFrame(data_exp, columns=["ID", "Nomor Surat", "Perihal", "Tujuan", "Tanggal", "Path File"])
    ...
    def aksi_tambah(self):
        ...
        if dialog.exec():
            # 1. Ambil data dari form (Gunakan ent_pihak)
            nomor = dialog.ent_nomor.text().strip()
            perihal = dialog.ent_perihal.text().strip()
            tujuan = dialog.ent_pihak.text().strip() 
            ...
            try:
                ...
                db = connect_db()
                cursor = db.cursor()
                # 2. Gunakan variabel lokal yang sudah diambil di atas
                cursor.execute("INSERT INTO surat (nomor_surat, judul_surat, asal_surat, kategori, tanggal, file_path) VALUES (%s,%s,%s,'keluar',%s,%s)",
                               (nomor, perihal, tujuan, tgl, path_dest))
                db.commit()
                db.close()
                self.load_data()
                self.notifikasi_custom("Berhasil", "Data disimpan!", QMessageBox.Icon.Information)
            except Exception as e: self.notifikasi_custom("Error", str(e), QMessageBox.Icon.Critical)

    def aksi_hapus(self):
        row = self.table.currentRow()
        if row < 0:
            # Gunakan notifikasi custom agar seragam
            self.notifikasi_custom("Peringatan", "Pilih data yang ingin dihapus!", QMessageBox.Icon.Warning)
            return

        # Ambil ID dari data UserRole (kolom 0)
        db_id = self.table.item(row, 0).data(Qt.ItemDataRole.UserRole)
        
        # 1. Kotak Konfirmasi (Harus dibuat manual karena butuh tombol Yes/No)
        msg = QMessageBox(self)
        msg.setWindowTitle("Konfirmasi Hapus")
        msg.setText("Apakah Anda yakin ingin menghapus data arsip ini?")
        msg.setIcon(QMessageBox.Icon.Question)
        msg.setStandardButtons(QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        
        # PERBAIKAN STYLE: Tambahkan warna hitam untuk QPushButton
        msg.setStyleSheet("""
            QMessageBox { background-color: white; }
            QLabel { color: black; font-size: 13px; }
            QPushButton { color: black; min-width: 80px; }
        """)
        ...
        if msg.exec() == QMessageBox.StandardButton.Yes:
            try:
                ...
                if db:
                    ...
                    # 3. Beri notifikasi sukses menggunakan fungsi custom
                    self.notifikasi_custom("Berhasil", "Data telah dihapus dari arsip.", QMessageBox.Icon.Information)
                    
            except Exception as e:
                # 4. Beri notifikasi error menggunakan fungsi custom
                self.notifikasi_custom("Error", f"Gagal menghapus data: {str(e)}", QMessageBox.Icon.Critical)
    ...
    def aksi_edit(self, data):
        ...
        # SINKRONISASI DENGAN ent_pihak
        dialog.ent_nomor.setText(str(data[1]))
        dialog.ent_perihal.setText(str(data[2]))
        dialog.ent_pihak.setText(str(data[3])) # Ganti ent_tujuan jadi ent_pihak
